...
async def on_ready():
    ...
    try:
        for guild in bot.guilds:
            ...
            print(f"Synced {len(synced)} admin slash commands cleanly for {guild.name}")
    except Exception as e:
...
async def slash_help(interaction: discord.Interaction):
    ...
    embed.add_field(name="1. /setup_verify", value="ส่งการ์ดปุ่มกดรับยศยืนยันตัวตนในช่อง ✅︱ᴠᴇʀɪꜰʏ", inline=False)
    ...
    embed.add_field(name="4. /setup_bot_status", value="ส่งและปักหมุดการ์ดมอนิเตอร์เช็กสถานะบอททุกตัวในช่อง 📊︱ʙᴏᴛ-ꜱᴛᴀᴛᴜꜱ", inline=False)
    embed.add_field(name="5. /setup_bot_roles", value="สร้างและมอบยศเฉพาะตัวให้บอททุกตัว (1 บอท : 1 ยศ)", inline=False)
    embed.add_field(name="6. /organize_existing_server", value="ย้ายจัดระเบียบช่องเดิมเป็น Smallcaps และรวม 15 ฟีดขยะเหลือ 4 ช่องหลัก", inline=False)
    embed.add_field(name="7. /clear_channel", value="ล้างข้อความค้างเก่าในช่องปัจจุบันให้สะอาด", inline=False)
    embed.add_field(name="8. /clean_webhooks", value="ลบและเคลียร์ Webhooks ขยะที่ค้างซ้ำซ้อนออกทั้งหมด", inline=False)
    embed.add_field(name="9. /backup_server", value="สำรองข้อมูลโครงสร้างเซิร์ฟเวอร์ปัจจุบันลง server_backup.json", inline=False)
    embed.add_field(name="10. /restore_server", value="กู้คืนข้อมูลโครงสร้างเซิร์ฟเวอร์จากไฟล์สำรอง", inline=False)
...
async def slash_setup_rules(interaction: discord.Interaction):
    ...
    channel = None
    for ch in guild.text_channels:
        if "rule" in ch.name.lower() or "ʀᴜʟᴇꜱ" in ch.name:
            channel = ch
            break

    if not channel:
        channel = await category.create_text_channel("📜︱ʀᴜʟᴇꜱ")
    else:
    ...
    embed.set_footer(text="โปรดปฏิบัติตามกฎระเบียบของเซิร์ฟเวอร์อย่างเคร่งครัด")

    msg = await channel.send(embed=embed)
    try: await msg.pin()
    except Exception: pass

    await interaction.response.send_message(f"✅ **ส่งการ์ดกฎระเบียบและปักหมุดเรียบร้อยแล้ว!**", ephemeral=True)
...
async def slash_setup_bypass_guide(interaction: discord.Interaction):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "🛡️ **คำแนะนำ:**\n"
            "• ระบบรองรับเฉพาะบางลิงก์ (ไม่สามารถปลดล็อกได้ทุกลิงก์)"
        ),
        color=discord.Color.green()
    )
    ...
    await interaction.response.send_message(f"✅ **ส่งการ์ดคู่มือ Zen Bypass เรียบร้อยแล้ว!**", ephemeral=True)
...
async def slash_setup_bot_status(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(f"✅ **สร้างการ์ดมอนิเตอร์สถานะบอทเรียบร้อยแล้ว!**", ephemeral=True)
...
async def slash_clear_channel(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.followup.send(f"🧹 **ล้างและลบข้อความเรียบร้อยแล้ว! ({len(deleted)} ข้อความ)**", ephemeral=True)
    except Exception as e:
